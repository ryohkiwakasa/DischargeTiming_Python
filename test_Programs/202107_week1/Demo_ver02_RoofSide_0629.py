...
RBfilepath = '../../Data/Part_A/Roof_side/Ver2_0625/DEMO_ROOF_SIDE_0625_3.JBI'
PBfolderpath = '../../Data/Part_A/Roof_side/Ver2_0625/PaintData/DEMO_ROOF_SIDE_0625_3PAINT_'
speed_filepath = '../../Data/Part_A/Roof_side/Ver2_0625/demo_roof_side_0625_3.csv'
PBfilepath = [PBfolderpath + str(i) + '.JBI' for i in range(62)]
...
#
# i = 1
# with open("wait_head_time_RoofSide.csv", 'w',  newline="") as f:
#     writer = csv.writer(f)
#     for time_print in wait_time:
...
# out_filename = "nozzle_" + str(sim_target_nozzle+1) + "_Roofside.xlsx"
# print(out_filename)
...
# filename = '../../3DModel/ASSY_hokan.stl'
...
# plotter.add_mesh(pv.PolyData(SPData_Re),
#                  scalars=Re_SP.Speed, cmap="cool", point_size=4, render_points_as_spheres=True, show_scalar_bar=True)

# plotter.add_mesh(pv.PolyData(NozzlePos), color="gray", point_size=2, render_points_as_spheres=True, show_scalar_bar=True)


# plotter.add_mesh(pv.PolyData(RobotData_Re + np.array([0, 0, 20])),
#                  scalars=ReData_RB.Speed, cmap="cool", point_size=4, render_points_as_spheres=True, show_scalar_bar=True)
...
# plotter.add_mesh(pv.PolyData(RobotData_Raw), color="red", point_size=6, render_points_as_spheres=True, show_scalar_bar=True)
# plotter.add_mesh(pv.PolyData(FigData_Raw), color="green", point_size=3, render_points_as_spheres=True, show_scalar_bar=True)
plotter.add_mesh(pv.PolyData(FigData_Re), color="green", point_size=3, render_points_as_spheres=True, show_scalar_bar=True)
# plotter.add_mesh(pv.PolyData(RobotData_Re), color="gray", point_size=3, render_points_as_spheres=True, show_scalar_bar=True)

# for i in range(5):
#     index = np.where(Raw_PD.sub_label%5 == i)[0]
#     plotter.add_mesh(pv.PolyData(FigData_Raw[index, :]), color=color_list[i], point_size=4, render_points_as_spheres=True, show_scalar_bar=True)

# for i in range(5):
#     index = np.where(ReData_PB.sub_label%5 == i)[0]
#     plotter.add_mesh(pv.PolyData(RobotData_Re[index, :]), color=color_list[i], point_size=4, render_points_as_spheres=True, show_scalar_bar=True)
